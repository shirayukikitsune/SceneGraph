...
Scene::updateCallback::auto_remover_type
Scene::addPreUpdateEvent(const Scene::updateCallback::function_type &function) {
    return preUpdateEvents.push_auto(function);
}
...
void Scene::removeTaggedNode(const std::string &Tag, std::shared_ptr<kitsune::scenegraph::Node> Node) {
    ...
    auto tagRange = TaggedNodes.equal_range(Tag);
