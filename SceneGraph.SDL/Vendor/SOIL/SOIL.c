...
#define SOIL_TEXTURE_WRAP_R					GL_TEXTURE_WRAP_R
#define SOIL_CLAMP_TO_EDGE					GL_CLAMP_TO_EDGE
...
int query_tex_rectangle_capability( void )
{
	...
	{
		/*	we haven't yet checked for the capability, do so	*/
		if(!GLAD_GL_ARB_texture_rectangle || !GLAD_GL_NV_texture_rectangle) {
			/*	not there, flag the failure	*/
...
int query_cubemap_capability( void )
{
	...
	{
		/*	we haven't yet checked for the capability, do so	*/
		if(!GLAD_GL_ARB_texture_cube_map || !GLAD_GL_EXT_texture_cube_map)
		{
			/*	not there, flag the failure	*/
...
int query_DXT_capability( void )
{
	...
	if( has_DXT_capability == SOIL_CAPABILITY_UNKNOWN )
	{
		...
		} else
		{
			...
			if( NULL == glad_glCompressedTexImage2DARB)
			{
			...
			{
				/*	all's well!	*/
				soilGlCompressedTexImage2D = glad_glCompressedTexImage2DARB;
